defmodule Slack.Socket do
  ...
  def handle_frame({:text, msg}, state) do
    case Jason.decode(msg) do
      ...
      # Interactive payloads (block actions, shortcuts, views, etc.)
      {:ok, %{"payload" => %{"type" => payload_type} = payload} = msg}
      when payload_type != "event" ->
        Logger.debug("[Slack.Socket] interactive message: #{inspect(msg)}")

        Task.Supervisor.start_child(
          {:via, PartitionSupervisor, {Slack.TaskSupervisors, self()}},
          fn -> handle_slack_event(payload_type, payload, state.bot) end
        )
